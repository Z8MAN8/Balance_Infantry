...
//static dji_motor_object_t *sht_motor[SHT_MOTOR_NUM];  // 发射器电机实例
 dji_motor_object_t *sht_motor[SHT_MOTOR_NUM];  // 发射器电机实例
...
static void shoot_pub_push(){
    mcn_publish(MCN_HUB(shoot_fdb), &shoot_fdb);
}
/**
...
static void shoot_sub_pull(){
    ...
    {
        mcn_copy(MCN_HUB(shoot_cmd), shoot_cmd_node, &shoot_cmd);
    }
}
